...
describe('GET/POST /api/account', () => {
  ...
  it('sets the preference and reflects it back on the next GET', async () => {
    ...
    const postRes = await request(app).post('/api/account/preferences').set('Cookie', cookie).send({ topXBo5: 9 });
    expect(postRes.status).toBe(200);
    expect(postRes.body.topXBo5).toBe(9);

    const getRes = await request(app).get('/api/account').set('Cookie', cookie);
  ...
  it('clears the preference when sent null', async () => {
    ...
    await request(app).post('/api/account/preferences').set('Cookie', cookie).send({ topXBo5: 4 });

    const clearRes = await request(app).post('/api/account/preferences').set('Cookie', cookie).send({ topXBo5: null });
  ...
  it('rejects a non-positive or non-integer topXBo5', async () => {
    ...
    for (const bad of [0, -3, 1.5, 'nine']) {
      const res = await request(app).post('/api/account/preferences').set('Cookie', cookie).send({ topXBo5: bad });
