...
/**
 * Anyone can look at a bracket; only its TOs can change it. The app used to
 * open the full reporting form regardless, so a spectator could fill the whole
 * thing in and only find out at the end.
 */
describe('ReportPanel — a set opened by someone who cannot report it', () => {
  const submitBtn = () => [...document.querySelectorAll('button')].find((b) => /report set/i.test(b.textContent ?? ''));
  ...
  afterEach(() => onQueue.mockReset());

  it('says why, rather than offering a button that cannot work', () => {
    renderReadOnly();

    expect(screen.getByText(/not a TO for this tournament/i)).toBeInTheDocument();
