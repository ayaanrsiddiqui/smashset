...
- Stations: `assignStation(setId, stationId)` takes a station **id**, and is independent of
  `markSetInProgress` — assigning leaves the set's state alone, so starting a set at a station is
  two mutations.
- `Tournament.stations` / `Event.stations` **page correctly** (`perPage: 10` gives 1-10 then 11-16),
...
- There is no root station query; stations come from `Tournament.stations`, `Event.stations`, or
  `Set.station`. `SetFilters.stationNumbers` filters sets by station **number**, verified working.
